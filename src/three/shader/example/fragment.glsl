...
// void main() {
//     // //网格绘制法
//     // float strength = step(0.02,mod(vUv.y * 10.0, 1.0));
//     // strength *= step(0.02,mod(vUv.x * 10.0, 1.0));
//     // float cross = step(0.02,mod((vUv.y) * 10.0, 1.0));
//     // cross += step(0.02,mod(vUv.x * 10.0, 1.0));
//     // // gl_FragColor = vec4 (0.0,1.0,1.0,(1.0 - strength) - 0.5 + (1.0 - cross));
//     // gl_FragColor = vec4 (0.0,1.0,1.0,(1.0 - strength)-0.5);
//     // if(strength == cross) {
...
{
	return hsv2rgb_smooth(vec3((f-(1.25/6.))*(1.25/6.),f*1.25+.2,f*.95));
}
...
void main ()
{
	...
	gl_FragColor = vec4(vec3(n).x,vec3(n).x,vec3(n).x,vec3(n).x);
}
