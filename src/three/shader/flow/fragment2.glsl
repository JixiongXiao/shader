...
//Animation
#define WATER_SPEED 2.5
#define ANIMATION_SPEED 1.0

float hash1_2(vec2 x)
{
 	return fract(cos(dot(x, vec2(52.127, 61.2871))) * 521.582);   
}

vec2 hash2_2(vec2 x)
{
    return fract(cos(x * mat2x2(20.52, 24.1994, 70.291, 80.171)) * 492.194);
}	
...
void main()
{
    ...
    gl_FragColor = vec4(res) * WATER_COLOR + fractVoro;
}